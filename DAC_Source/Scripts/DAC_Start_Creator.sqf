...
if(DAC_Code < 2) then
{
	...
	[] execVM "\DAC_Source\Scripts\DAC_Vehicle_check.sqf";
	...
	else
	{
		...
		if(DAC_Marker > 0) then {[]execVM "\DAC_Source\Marker\DAC_InitMarker.sqf"};
	};
}
else
{
	...
	if(DAC_Code == 3) then
	{
		...
		if(DAC_Marker > 0) then {if(local player) then {[player,1,(group player)] execVM "\DAC_Source\Marker\DAC_GroupMarker.sqf";[]execVM "\DAC_Source\Marker\DAC_InitMarker.sqf"}};
